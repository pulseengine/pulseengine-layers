...
class TheRealManifest(unittest.TestCase):
    ...
    def test_the_hub_entry_in_the_live_manifest_is_recognised_as_one(self):
        path = pathlib.Path(__file__).parent.parent / "layer.toml"
        with open(path, "rb") as f:
            m = tomllib.load(f)
        hubs = [
        ...
        self.assertIn(
            "with-device", hubs,
            "the known hub payload is no longer recognised — either the manifest changed "
            "or the rule did, and the scanner would sign a version the payload lacks",
        )
